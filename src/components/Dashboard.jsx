...
import { fetchProgressForDoc, fetchSignoffsForDoc, progressArrayToMap, signoffArrayToMap } from '../lib/api';
    ...
    async function load() {
      const stats = await Promise.all(
        documents.map(async (doc) => {
          ...
          const progressMap = progressArrayToMap(progressRows);
          const signoffMap = signoffArrayToMap(signoffRows);
          ...
          const signedOffCount = Object.keys(signoffMap).length;
          ...
          return { doc, totalProcesses, completed, started, notStarted, signedOffCount, pct, nextAction };
        })
      );
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <SummaryCard icon={<TrendingUp className="w-5 h-5 text-teal-500" />} label="Overall Progress" value={`${overallPct}%`} sublabel={`${overallCompleted}/${overallTotal} processes`} />
        <SummaryCard icon={<BookOpen className="w-5 h-5 text-blue-500" />} label="In Progress" value={docStats.reduce((s, d) => s + d.started, 0)} sublabel="actively learning" />
        <SummaryCard icon={<CheckCircle2 className="w-5 h-5 text-emerald-500" />} label="Completed" value={overallCompleted} sublabel="confident or mastered" />
        <SummaryCard icon={<Award className="w-5 h-5 text-purple-500" />} label="Signed Off" value={overallSignedOff} sublabel="by manager" />
      </div>
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-slate-500 mt-0.5">{doc.groups?.length || 0} groups &middot; {totalProcesses} processes</p>
                </div>
                ...
                <div className="flex items-start gap-2 pt-3 border-t border-slate-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      ...
                      <span className={getGrade(nextAction.grade).textClass}>{getGrade(nextAction.grade).label}</span>
                    </p>
                  </div>
...
function ManagerDashboard({ documents, users, currentUser, onNavigate }) {
  ...
  useEffect(() => {
    ...
    async function load() {
        ...
        trainees.map(async (trainee) => {
          ...
          const docDetails = await Promise.all(
            documents.map(async (doc) => {
              ...
              let completed = 0;
              const signedOff = Object.keys(signoffMap).length;
          ...
          return { trainee, totalProcesses, totalCompleted, totalSignedOff, overallPct, needsAttention, docDetails };
        })
      );
      ...
      if (!cancelled) setTraineeStats(stats);
    }
    load();
    return () => { cancelled = true; };
  }, [documents, trainees.length]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <SummaryCard icon={<Users className="w-5 h-5 text-blue-500" />} label="Trainees" value={trainees.length} sublabel="active" />
        <SummaryCard icon={<FileText className="w-5 h-5 text-teal-500" />} label="Documents" value={documents.length} sublabel="training docs" />
        <SummaryCard icon={<AlertTriangle className="w-5 h-5 text-amber-500" />} label="Need Review" value={traineeStats.filter((t) => t.needsAttention).length} sublabel="awaiting sign-off" />
      </div>
      ...
      {trainees.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {traineeStats.map(({ trainee, totalProcesses, totalCompleted, totalSignedOff, overallPct, needsAttention, docDetails }) => (
            <div
              key={trainee.id}
              className={`bg-white rounded-lg border p-5 ${needsAttention ? 'border-amber-200 bg-amber-50/30' : 'border-slate-200'}`}
            >
              <div className="flex items-start justify-between mb-3">
              ...
              <div className="w-full bg-slate-100 rounded-full h-2 mb-3">
                <div className="bg-teal-500 h-2 rounded-full transition-all duration-300" style={{ width: `${overallPct}%` }} />
              </div>
